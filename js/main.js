...
(() => {
  ...
  function updateDOM(data) {
    ...
    // 2. About Section
    if (data.about && data.about.fields) {
      ...
      const expEl = $('#statExperience');
      if (expEl && about.stat_experience) expEl.dataset.count = about.stat_experience.replace(/\+/, '');
      const compEl = $('#statCompanies');
      if (compEl && about.stat_companies) compEl.dataset.count = about.stat_companies;
      const pubEl = $('#statPublications');
      if (pubEl && about.stat_publications) pubEl.dataset.count = about.stat_publications;
      const degEl = $('#statDegrees');
      if (degEl && about.stat_degrees) degEl.dataset.count = about.stat_degrees;
    }

    // 3. Work Section (Projects)
    if (data.work && data.work.items && data.work.items.length > 0) {
    ...
    if (data.experience && data.experience.items && data.experience.items.length > 0) {
      ...
      if (container) {
        ...
        data.experience.items.forEach(item => {
          ...
          const desc = item.fields.description || '';
          const bullets = item.bullets.map(b => `<li>${formatText(b)}</li>`).join('');
    ...
    // 5. Skills Section
    if (data.skills && data.skills.fields) {
    ...
    // 6. Research Section
    if (data.research && data.research.items && data.research.items.length > 0) {
    ...
    // 7. Education Section
    if (data.education && data.education.items && data.education.items.length > 0) {
    ...
    // 8. Contact Section Links
    if (data.contact && data.contact.fields) {
